...
/**
 * Payment is ALWAYS auto-verified — never self-claimed.
 *
 * Book & Pay → Razorpay Checkout (card / UPI / QR) → the payment signature is
 * verified on the server (/api/payment/verify), and the Razorpay webhook
 * (/api/webhooks/razorpay) confirms it server-to-server as a backup. A session
 * only becomes PAID from one of those verified sources, so it can never show
...
export default function BookSessionButton({ slug, className = "btn-primary", label = "Book & Pay" }: Props) {
  ...
  async function handleClick() {
    ...
    try {
      ...
      // 2. Auto-verify requires the Razorpay keys
      if (!RAZORPAY_KEY_ID) {
        toast.error("Online payment isn't enabled yet. Please contact the studio.");
        return;
      }

      // 3. Create the order (+ a PENDING booking tied to this order id)
      ...
      // 4. Open Razorpay Checkout — card / UPI / scan-QR, all inside Razorpay
      const rzp = new window.Razorpay({
        ...
        handler: async (resp: any) => {
          // 5. Verify signature server-side before anything is marked paid
